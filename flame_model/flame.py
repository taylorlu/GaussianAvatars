...
class FlameHead(nn.Module):
    ...
    def __init__(
        ...
        parents = to_tensor(to_np(flame_model.kintree_table[0])).to(torch.int32)
        parents[0] = -1
    ...
    def add_teeth(self):
        ...
        # update vertex masks
        self.mask.v.register_buffer("teeth_upper", vid_teeth_upper.to(torch.int32))
        self.mask.v.register_buffer("teeth_lower", vid_teeth_lower.to(torch.int32))
        self.mask.v.register_buffer("teeth", vid_teeth.to(torch.int32))
...
class FlameMask(nn.Module):
    ...
    def process_vertex_mask(self, flame_parts_path):
        ...
        for k, v_mask in part_masks.items():
            self.v.register_buffer(k, torch.tensor(v_mask, dtype=torch.int32))
    ...
    def create_custom_mask(self):
        ...
        self.v.register_buffer("neck_left_point", torch.tensor([3193], dtype=torch.int32))
        self.v.register_buffer("neck_right_point", torch.tensor([3296], dtype=torch.int32))
        self.v.register_buffer("front_middle_bottom_point_boundary", torch.tensor([3285], dtype=torch.int32))
        self.v.register_buffer("back_middle_bottom_point_boundary", torch.tensor([3248], dtype=torch.int32))
        ...
        self.v.register_buffer("lip_inside_ring", torch.cat([self.v.lip_inside_ring_upper, self.v.lip_inside_ring_lower, torch.tensor([1594, 2730], dtype=torch.int32)]))
        ...
        self.v.register_buffer("skin", skin.to(torch.int32))
    ...
    def process_face_mask(self, faces):
        ...
        for region_name, f_mask in face_masks.items():
            self.f.register_buffer(region_name, torch.tensor(f_mask, dtype=torch.int32))
    ...
    def process_face_clusters(self, face_clusters):
        """ Construct a lookup table from face id to cluster id.
            ...
            ...
        """
        fid2cid = torch.ones(self.num_faces+1, dtype=torch.int32)  # faces are always treated as foreground
    ...
    def process_vt_mask(self, faces, faces_t):
        ...
        for region_name, vt_mask in vt_masks.items():
            self.vt.register_buffer(region_name, torch.tensor(vt_mask, dtype=torch.int32))
